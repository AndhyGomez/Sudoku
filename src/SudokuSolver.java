...
public class SudokuSolver
{
			...
			for(int possibleValueIndex = 0; possibleValueIndex < possibleValues.length; possibleValueIndex++)
			{
				for(int row = 0; row < puzzle.length; row++)
				{
					for(int col = 0; col < puzzle[0].length; col++)
					{
						...
						{													
							// Cases for top 3 quadrants
							if(row < 3 && col < 3)
							{
								boxCheck = check3x3(puzzle, 0, 3, 0, 3, possibleValues[possibleValueIndex]);
							}
							if(row < 3 && col > 2 && col < 6)
							{
								boxCheck = check3x3(puzzle, 0, 3, 3, 6, possibleValues[possibleValueIndex]);
							}
							if(row < 3 && col > 5)
							{
								boxCheck = check3x3(puzzle, 0, 3, 6, 9, possibleValues[possibleValueIndex]);
							}
							
							// Cases for middle 3 quadrants
							if(row > 2 && row < 6 && col < 3)
							{
								boxCheck = check3x3(puzzle, 3, 6, 0, 3, possibleValues[possibleValueIndex]);
							}
							if(row > 2 && row < 6 && col > 2 && col < 6)
							{
								boxCheck = check3x3(puzzle, 3, 6, 3, 6, possibleValues[possibleValueIndex]);
							}
							if(row > 2 && row < 6 && col > 5)
							{
								boxCheck = check3x3(puzzle, 3, 6, 6, 9, possibleValues[possibleValueIndex]);
							}
							
							// Cases for bottom 3 quadrants
							if(row > 5 && col < 3)
							{
								boxCheck = check3x3(puzzle, 6, 9, 0, 3, possibleValues[possibleValueIndex]);
							}
							if(row > 5 && col > 2 && col < 6)
							{
								boxCheck = check3x3(puzzle, 6, 9, 3, 6, possibleValues[possibleValueIndex]);
							}
							if(row > 5 && col > 5)
							{
								boxCheck = check3x3(puzzle, 6, 9, 6, 9, possibleValues[possibleValueIndex]);
							}
							...
							if(boxCheck == true && rowCheck == true && colCheck == true )
							{	
								viability[row][col] = true;
							}
							
							// Determine if there is only one possibility
	 ...
	 * @param minRow Min value for row to be used in for loop
	 * @param maxRow Max value for row to be used in for loop
	 * @param minCol Min value for col to be used in for loop
	 * @param maxCol Max value for col to be used in for loop
	 * @param valueToCheck value to look for within 3x3
	 * @return
	 */
	public static boolean check3x3(String[][] array, int minRow, int maxRow, int minCol, int maxCol, String valueToCheck)
	{
		...
		A: for(int row = minRow; row < maxRow; row++)
			{
